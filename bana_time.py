...
while True:
    ...
    bananas = faceCascade.detectMultiScale(
        ...
        scaleFactor=1.3,   # parameter specifying how much the image size should be reduced at each
                           # image scale, used to create scale pyramid.

        minNeighbors=20,   # specifying how many neighbors each candidate rectangle should have,
                           # to retain it. Higher number gives lower false positives.

        minSize=(10, 10)   # minimum rectangle size to be considered a face.
    )

    for (x,y,w,h) in bananas:  # marks bananas  with a rectangle.
